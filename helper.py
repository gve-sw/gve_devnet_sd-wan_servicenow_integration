...
def send_notification(action, sys_id, full_log):
    ...
    try:
        ...
        return "ERROR: Retrieving the WEBEX TOKEN, check your enviromental variables" + str(e)

    try:    
        to = os.environ['WEBEX_ALERT_USER']
    except Exception as e:
        return "ERROR: Retrieving the WEBEX ALERT USER, check your enviromental variables" + str(e)
    
    try:
        card_template =  get_json_card("servicenow.json")
    except Exception as e:
        return "ERROR: While retrieving the CARD TEMPLATE, check the servicenow.json" + str(e)
    ...
    try:
        ...
        return "ERROR: While sending the Notification" + str(e)
...
def create_incident(category,impact,desc,urgency,vmanage_alert_id):
    ...
    ticket = {
        ...
        "urgency": urgency,
        "category": category,
        "correlation_id": vmanage_alert_id,
        "short_description": desc,
        "description": "The full log for this alert is:" + desc
    }

    try:
        create_ticket = requests.post(os.environ['SERVICENOW_INSTANCE'] + "/api/now/table/incident", auth=auth, headers=headers, json=ticket)
    except Exception as e:
        print("Error on Creating ServiceNow Ticket! Check your ServiceNow settings or connectivity!")
    ...
    if os.environ['WEBEX_NOTIFICATION'] == '1':
        try:
            ...
            return "Error on Sending Webex Message!If it is not intended to use turn it from enviromental variables!" + str(e)
...
def get_incident(vmanage_alert_id): 
    ...
    try:
        ...
        print("Error on Getting ServiceNow Incidents! Check your ServiceNow settings or connectivity")
    ...
    for items in servicenow_raw_json:
        if items['correlation_id'] == str(vmanage_alert_id):
            return items['sys_id']
